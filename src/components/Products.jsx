...
const Products = () => {
  ...
  const handleCart = (product) => {
    ...
    notify(`${product.title.slice(0, 10)}...  added to cart`);
  }
      ...
      <div className="btns flex flex-wrap items-center justify-center gap-3 my-10">
        {cateBtn.map((btn, index) => {
          return (
            <button
              key={index}
              className={`text-black hover:underline capitalize ${btn === selItem ? 'font-bold' : ''}`}
              onClick={() => setSelItem(btn)}
            >
              {btn}
            </button>
          );
          ...
          <div className="productsWarrper flex flex-wrap gap-4 justify-center pb-12">
            {filterValue.map((product) => {
              ...
              return (
                <div
                  key={product.id}
                  className="border product group hover:shadow-md flex duration-300 flex-col hover:shadow-blue-400 hover:translate-y-[-8px] transition-all h-[22rem] rounded-2xl w-60  py-3 px-1 bg-white overflow-hidden relative"
                >
                  <div
                    onClick={() => {
                      handleCart(product);
                  ...
                  <div className="content h-[40%]  pt-4 px-3">
                    ...
                    <Link
                      ...
                      className="font title text-xs hover:underline group-hover:text-cyan-700 transition-all duration-300"
                    >
